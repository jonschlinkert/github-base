...
function interpolate(str, context) {
  ...
  let val = opts.apiurl + str.replace(/:([\w_]+)/g, function(m, key) {
    union(opts.params, key);
    return has(opts, key) ? get(opts, key) : key;
  });

  if (opts.method.toLowerCase() === 'get' && opts.paged !== false) {
    const obj = url.parse(val);
    const query = obj.query ? qs.parse(obj.query) : {};
    const noquery = omit(Object.assign({}, obj), ['query', 'search']);
...
function result(res, data, opts, options) {
  if (res.statusCode >= 400) {
    ...
    Reflect.defineProperty(err, 'response', { value: res, enumerable: false });
...
function badCredentials(res) {
  return res && res.message === 'Bad credentials';
}

function maxLogins(res) {
  return res && res.message && /login attempts/i.test(res.message);
}
